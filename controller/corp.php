    ...
    case 'updateInfoCorp':
        ...
        if (isset($_POST['updateInfoCorp'])) {
            ...
            if ($same == '') {
                ...
                for ($i = 1; $i <= 5; $i++) {
                    if (isset($_FILES['thumb' . $i]) && ($_FILES['thumb' . $i]['size'] <= 10000000)) {
                        $file = $_FILES['thumb' . $i];
                ...
                if (isset($_FILES['avatar']) && ($_FILES['avatar']['size'] <= 5000000)) {
                    $file = $_FILES['avatar'];
                    // sử dụng str_replace để xóa các khoảng trắng, tránh lỗi chèn src có space trong thẻ img
            ...
            echo $overSizeAvatar == 1 ? "<script>alert('[ Ảnh đại diện vượt quá kích thước cho phép - 5 MB]')</script>" : '';
            echo $overSizeThumb == 1 ? "<script>alert('[ Ảnh giới thiệu vượt quá kích thước cho phép - 10 MB]')</script>" : '';
