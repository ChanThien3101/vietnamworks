<footer id="footer" class="footer-area three pt-100 pb-70">
    <div class="container">
        <div class="row">
            <div class="col-sm-6 col-lg-5">
                <div class="footer-item three">
                    <div class="footer-logo">
                        <a class="logo" href="index.html">
                            <img data-cfsrc="assets/img/logo-three.png" alt="Logo" style="display:none;visibility:hidden;"><noscript><img src="assets/img/favicon.png" alt="Logo"></noscript>
                        </a>
                        <ul>
                            <li>
                                <span>Địa chỉ: </span>
                                - Trụ sở chính:
                              277 Quan Hoa, Cầu Giấy, Hà Nội
                                             - Chi nhánh Nhật Bản:
                              Tầng 6, Tòa nhà Nishi-Shinjuku Mizuma ,
                              3-3-13 Nishi-Shinjuku, Shinjuku-ku, Tokyo. Hà Nội
                            </li>
                            <li>
                                <span>Email : </span>
                                <a href="https://templates.hibootstrap.com/cdn-cgi/l/email-protection#f8909d949497b8929d9b8c97d69b9795">contact@lisod.vn</a>
                            </li>
                            <li>
                                <span>Hà nội: </span>
                                <a href="tel:2151234567">0221897567</a>
                            </li>
                            <li>
                                <span>Hồ Chí Minh : </span>
                                <a href="tel:2151234567">0221897567</a>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        
            <div class="col-sm-6 col-lg-3 ps-5 pt-2">
                <div class="footer-item three">
                    <div class="footer-service">
                        <h3>Về LISOD</h3>
                        <ul>
                            <li>
                                <a href="index.php" target="_blank">Trang chủ</a>
                            </li>
                            <li>
                                <a href="index.php?act=listCorp" target="_blank">Nhà tuyển dụng</a>
                            </li>
                            <li>
                                <a href="index.php?act=listRecr" target="_blank">Công việc</a>
                            </li>
                            <li>
                                <a href="index.php?act=listCv" target="_blank">CV nổi bật</a>
                            </li>
                            
                            <li>
                                <a href="contact.html" target="_blank">Giới thiệu</a>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="col-sm-6 col-lg-4">
                <div class="footer-item three">
                    <div class="footer-newsletter">
                        <h3>Đăng kí thông báo</h3>
                        <p>Để lại email để nhận được các thông tin tuyển dụng mới nhất từ chúng tôi !</p>
                        <form class="newsletter-form" data-toggle="validator">
                            <input type="email" class="form-control" placeholder="Địa chỉ Email*" name="EMAIL" required autocomplete="off">
                            <button class="btn" type="submit">
                                <i class="flaticon-send"></i>
                            </button>
                            <div id="validator-newsletter" class="form-result"></div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>
</footer>

<div id="footPage" class="copyright-area three">
    <div class="container ">
        <div class="text-center ">

        </div>
    </div>

</div>

